...
func (c *Client) handleUDPTunnel(buffer []byte) error {
	...
	// mumble-web patch: allow raw voice packet interception (Opus passthrough,
	// no PCM decoding required). The handler must not block.
	if c.RawVoiceHandler != nil {
		c.RawVoiceHandler(buffer)
		return nil
	}
